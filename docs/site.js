...
(function () {
  ...
  /* ---------- hero reveal (landing only) ----------
     Own try/catch: a throw here must not stop execution from reaching the
     entrance observer below, or every section on the page stays invisible
     (AI11 — same reasoning for the so-what and formula-hover blocks). */
  try {
  ...
  } catch (err) { /* see block comment above — swallow and continue */ }
  ...
  } catch (err) { /* see hero-block comment above — swallow and continue */ }
  ...
  // Selector list must stay identical to the inline failsafe in each page's
  // <head> (index.html / methodology.html) — that copy can't import this one.
  ...
  if (reduce) {
    ...
    window.__gtEntrancesOwned = true;
    return;
  }
  ...
  // Claims ownership of the entrances, disarming the inline <head> failsafe
  // (AI11). Set only once the observer is actually wired: anything that throws
  // before this line leaves the flag unset, so the failsafe still fires and
  // the page can't strand content at opacity:0. Deliberately NOT a blanket
  // timer here — forcing every target visible after 2s would reveal all
  // below-fold content before the reader ever scrolls to it, silently
  // cancelling the scroll entrances this observer exists to drive.
  window.__gtEntrancesOwned = true;
})();
